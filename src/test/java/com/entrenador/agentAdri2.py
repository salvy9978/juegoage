...
penalizacionMetrosPorAngulo = 10
umbralRentaIrPorBola = 3000
anguloMaxParaHuir = 40
#velocidades
velocidadMinimaParaFrenar = 400
...
def getTurnosABola(coche, bola): # TODO: add velocidad - medio solucionado
    turnos = 0
    vel = math.sqrt(coche[4]**2+coche[5]**2)
    if(vel == 0):
        vel = epsilon

    turnos = turnos + (getDistancia(coche[2], coche[3], bola[2], bola[3])/vel)
    penalizacion = int(getDiferenciaAngulos(coche[6], coche[2], coche[3], bola[2], bola[3])/18) * penalizacionMetrosPorAngulo
    turnos = turnos + penalizacion
    #print("TURNOS CALC DE "+str(coche[0])+" A BOLA "+str(bola[0])+": "+str(turnos)+" PEN("+str(penalizacion)+")", file=sys.stderr, flush=True)
    return turnos
...
def queBolaBusco(coche, misTurnosABolas, turnosEnemigosABolas, bolas):
    id = -1
    minTurnosBolas = -1
    for i in range(len(bolas)): # TODO: hay que cambiar para que mire los turnos a la bola en concreto y no pille el minimo global - arreglado en teoria
       ...
       # if(turnosAux<min(turnosEnemigosABolas) and turnosAux<=min(misTurnosABolas) and (turnosAux<minTurnosBolas or minTurnosBolas==-1) and turnosAux<umbralRentaIrPorBola):
        ...
        hayEnemigosMasCercanos = False
        #se asume que el orden es el mismo
        #los turnos van por pares
        if(turnosEnemigosABolas[i]<turnosAux or turnosEnemigosABolas[i+1]<turnosAux):
            hayEnemigosMasCercanos = True
            #print("ENEMIGO MAS CERCANO A "+str(bolas[i][0])+" QUE "+str(coche[0]), file=sys.stderr, flush=True)

        if(turnosAux<=min(misTurnosABolas) and (turnosAux<minTurnosBolas or minTurnosBolas==-1) and hayEnemigosMasCercanos == False):
            id = i
...
def aQuienAtaco(coche, misCoches, cochesEnemigo, listaEnemigosAtacados):
    ...
    return id # TODO que no vayan los dos a por el mismo - creo que esta hecho
...
def deboIrCentro(coche, cochesEnemigo): # TODO: add si enemigo nos esta apuntando --> IMPORTANTE -solucionado
    coordenadas = [0,0]
    for i in cochesEnemigo:
        if(getDistancia(i[2], i[3], 0,0)<distanciaEnemigoEstaEnCentro and (getDiferenciaAngulos(i[6], i[2], i[3], coche[2], coche[3])<anguloMaxParaHuir)): # TODO: calcula coordenadas de huir, cambiar -solucionado
            pendiente = coche[3]/coche[2] #pendiente de la recta con el centro
            #print("Pendiente ID"+str(coche[0])+" = "+str(coche[3])+"/"+str(coche[2]), file=sys.stderr, flush=True)
            coordenadas[0] = 10000 #valor arbirtrario para que se mueva en una direccion
            coordenadas[1] = int((-1*pendiente)*coordenadas[0])
            return coordenadas
    return coordenadas
# game loop
while True:
    ...
    for i in range(car_count):
        ...
        if(misCoches[i][7]!=-1): #dejar bola si la tengo
            ...
            if(coordenadasIrConPelota[0]==0 and coordenadasIrConPelota[1]==0):
                ...
                distCent = getDistancia(misCoches[i][2], misCoches[i][3], 0, 0)
                if distCent < (3*center_radius):
                    aceleracion = aceleracion/ (distCent/center_radius)
                   # print("COCHE "+str(misCoches[i][0])+" CERCA MEDIO: "+str(distCent/center_radius), file=sys.stderr, flush=True)
                    coche = "DejarBolaCercaMedio"
            else:
                coche = "Huir"
            ...
            if(idBola!=-1): # cogerBola si se da una buena ocasion
                ...
                tiempoAprox = getDistancia(misCoches[i][2], misCoches[i][3], bolas[idBola][2], bolas[idBola][3])/(velocidad*2)
                #print("COCHE "+str(misCoches[i][0])+" VA A BOLA "+str(idBola)+" / T = "+str(tiempoAprox), file=sys.stderr, flush=True)
                dirX = bolas[idBola][2] + factorPrediccion*tiempoAprox*bolas[idBola][4]
                dirY = bolas[idBola][3] + factorPrediccion*tiempoAprox*bolas[idBola][5]
                ...
                if(getDistancia(bolas[idBola][2],bolas[idBola][3],misCoches[i][2],misCoches[i][3])<distanciaUmbralCogerBola):
                    vel = math.sqrt(math.pow(misCoches[i][4],2)+math.pow(misCoches[i][5],2))
                    if(vel > velocidadMinimaParaFrenar):
                        aceleracion = minAceleracionCogerBola # si estoy cerca frenar
                        #print("COCHE "+str(misCoches[i][0])+" VA A BOLA LENTO: "+str(vel), file=sys.stderr, flush=True)
                        coche = "CogerBolaLento"
                        #dirX = bolas[idBola][2]
                        #dirY = bolas[idBola][3]
                    else:
                        aceleracion = maxAceleracionCogerBola
                        coche = "CogerBolaLentoAceleron"
                    ...
                    penal = int(getDiferenciaAngulos(misCoches[i][6], misCoches[i][2], misCoches[i][3], bolas[idBola][2], bolas[idBola][3]))
                    aceleracion = int(aceleracion - factorPenal*penal)
                    coche = "CogerBolaRapido"
                    if(aceleracion < 0):
                        aceleracion = 10
                        coche = "CogerBolaRapidoGirando"
